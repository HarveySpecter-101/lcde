...
/* ─────────────────────────────────────────────────────────
   LAYER 1 — Canvas particle system — OPTIMIZED
   25 particles on desktop · 12 on mobile
   FPS-capped at 30, pauses when not visible
───────────────────────────────────────────────────────── */
...
function useParticleCanvas(sectionRef: React.RefObject<HTMLElement | null>) {
  ...
  useEffect(() => {
    ...
    const COUNT = isMobile ? 12 : 25; // Reduced from 60/30
    ...
    const draw = (timestamp: number) => {
      ...
      // Skip if hidden or section not visible
